...
def get_compliance_level(mandatory_rate, capability_rate, quality_rate, feature_rate):
    ...
    if (capability_rate >= full_featured['capability_success_rate'] and 
        quality_rate >= full_featured['quality_success_rate'] and 
        feature_rate >= full_featured['feature_success_rate']):
    ...
    if (capability_rate >= recommended['capability_success_rate'] and 
        quality_rate >= recommended['quality_success_rate']):
...
def get_next_level_requirements(current_level, mandatory_rate, capability_rate, quality_rate, feature_rate):
    ...
    return {
        'next_level': None,
        'gaps': ['Maintain current compliance level'],
        'priority': 'LOW'
    }
...
def generate_compliance_summary(mandatory_rate, capability_rate, quality_rate, feature_rate):
    ...
    return {
        ...
        'scores': {
            'mandatory': mandatory_rate,
            'capability': capability_rate, 
            'quality': quality_rate,
            'feature': feature_rate
        },
        'overall_score': (
            mandatory_rate * 0.50 +
            capability_rate * 0.25 +
            quality_rate * 0.15 +
            feature_rate * 0.10
        )
    } 
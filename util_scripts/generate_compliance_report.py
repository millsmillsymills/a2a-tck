...
class ComplianceReportGenerator:
    ...
    def generate_report(self) -> Dict:
        ...
        is_compliant = mandatory_compliance["passed"] == mandatory_compliance["total"] and mandatory_compliance["total"] > 0
    ...
    def _calculate_compliance(self, results: Dict) -> Dict:
        ...
        if not results:
            return {"total": 0, "passed": 0, "failed": 0, "skipped": 0, "xfailed": 0, "success_rate": 0, "failure_rate": 0}
    ...
    def _determine_compliance_level(self, mandatory, capability, quality, feature) -> Dict:
        ...
        if capability["success_rate"] >= 95 and quality["success_rate"] >= 90 and feature["success_rate"] >= 80:
            return COMPLIANCE_LEVELS["FULL_FEATURED"]

        # Check for recommended compliance
        if capability["success_rate"] >= 85 and quality["success_rate"] >= 75:
    ...
    def _generate_recommendations(self, mandatory, capability, quality, feature) -> List[str]:
        ...
        if mandatory["success_rate"] < 100:
            recommendations.append("🚨 CRITICAL: Fix mandatory test failures to achieve A2A compliance")
            recommendations.append(f"Focus on {mandatory['failed']} failing mandatory tests first")

        if capability["success_rate"] < 90:
            recommendations.append("⚠️ HIGH: Address capability validation failures - these indicate false advertising")

        if quality["success_rate"] < 80:
            recommendations.append("💡 MEDIUM: Improve quality test results for production readiness")

        if feature["success_rate"] < 70:
            recommendations.append("📈 LOW: Consider implementing more optional features for completeness")
...
def generate_report_from_file(results_file: str, output_file: str, agent_card_file: str = None):
    ...
    if agent_card_file and Path(agent_card_file).exists():
        with open(agent_card_file) as f:

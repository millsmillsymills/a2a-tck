...
class TestValidJSON:
    ...
    def test_output_is_valid_json(
        self, collector: CompatibilityCollector, formatter: JSONFormatter
    ) -> None:
...
class TestSummary:
    ...
    def test_compatibility_are_percentage_strings(
        self, collector: CompatibilityCollector, formatter: JSONFormatter
    ) -> None:
        """Compatibility values are strings ending with '%'."""
        collector.record(requirement_id="R1", transport="http", passed=True, level="MUST")
        report = CompatibilityAggregator(collector).aggregate()
        data = json.loads(formatter.format(report))
        summary = data["summary"]
        for key in ("overall_compatibility", "must_compatibility", "should_compatibility", "may_compatibility"):
...
class TestAgentCardInJSON:
    ...
    def test_agent_card_included(
        ...
        report = CompatibilityAggregator(collector, agent_card=card).aggregate()
        data = json.loads(formatter.format(report))
...
class TestWriteFile:
    ...
    def test_write_creates_file(
        self, collector: CompatibilityCollector, formatter: JSONFormatter, tmp_path: Path
    ) -> None:
        """write() creates parent dirs and writes valid JSON."""
        collector.record(requirement_id="R1", transport="http", passed=True, level="MUST")
        report = CompatibilityAggregator(collector).aggregate()
        out = tmp_path / "reports" / "compatibility.json"

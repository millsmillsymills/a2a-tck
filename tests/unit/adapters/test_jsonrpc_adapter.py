...
class TestJSONRPCTestAdapter:
    ...
    def test_get_task_with_wrong_task_id(self, adapter, test_context):
        ...
        adapter.jsonrpc_client.get_task = Mock(return_value={
    ...
    def test_raw_json_rpc_call(self, adapter, test_context):
        ...
        result = adapter.test_raw_json_rpc_call(
            test_context, 
            "custom/method", 
            {"param1": "value1"}
        )
    ...
    def test_error_handling_with_transport_error(self, adapter, test_context):
        ...
        adapter.jsonrpc_client.send_message = Mock(
            side_effect=TransportError("Network error", TransportType.JSON_RPC)
        )

...
def test_invalid_method_error_validation(sut_client):
    ...
    assert error_analysis["has_error_code"], \
        "Invalid method error must include error code"
    
    # MANDATORY: Error code should be -32601 (Method not found)
    expected_code = -32601
    actual_code = error_analysis["error_code"]
    assert actual_code == expected_code, \
        f"Invalid method should return error code {expected_code}, got {actual_code}"
    
    # MANDATORY: Must have error message
    assert error_analysis["has_error_message"], \
        "Invalid method error must include error message"
        ...
        for test in tests:
            ...
            try:
                ...
                if "error" in response:
                    ...
                    logger.info(f"  {test['method']}: code={error_analysis['error_code']}, message='{error_analysis['error_message']}'")
                else:

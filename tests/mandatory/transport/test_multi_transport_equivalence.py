...
def test_message_sending_equivalence(transport_capabilities):
    ...
    assert len(successful_transports) > 0, \
        f"Message sending failed on all transports: {failed_transports}"
...
def test_task_retrieval_equivalence(transport_capabilities):
    ...
    assert len(successful_retrievals) > 0, \
        f"Task retrieval failed on all transports for task {task_id}"
...
def test_agent_card_access_equivalence(transport_capabilities):
    ...
    assert len(successful_retrievals) > 0, \
        "Agent Card access failed on all transports"
    ...
    error_scenarios = [
        {
            ...
            "operation": "get_task",
            "params": {"task_id": "invalid-nonexistent-task-id-12345"}
        },
        {
...
def test_performance_equivalence(transport_capabilities):
    ...
    for operation in perf_operations:
        ...
        for transport in available_transports:
            ...
            if response_times:
                ...
                logger.info(f"✅ {transport}: Avg {avg_response_time:.3f}s, Min {min_response_time:.3f}s, Max {max_response_time:.3f}s")
            else:
                operation_results[transport] = {
    ...
    for operation_name, operation_results in transport_performance.items():
        ...
        successful_transports = [t for t, r in operation_results.items() 
        ...
        slow_transports = [t for t in successful_transports 
                          if operation_results[t]["avg_response_time"] > 5.0]
...
def test_concurrent_operation_equivalence(transport_capabilities):
    ...
    assert len(successful_concurrent) > 0, \
        "No concurrent operations succeeded across any transport"

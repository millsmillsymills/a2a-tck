        ...
        if response.status_code in (401, 403):
            ...
            try:
                ...
                if "error" in json_response:
                    ...
                    if error_code in (-32600, -32601, -32602, -32603) or \
                       any(keyword in error_message.lower() for keyword in ["auth", "unauthorized", "forbidden"]):
                        pytest.fail(
    ...
    if "security" in auth_agent_card:
        ...
        for req_i, requirement in enumerate(security_requirements):
            ...
            for scheme_name in requirement.keys():
                scheme_declared = any(
                    declared_name == scheme_name 
                    for declared_name in (auth_agent_card.get("securitySchemes", {})).keys()
                )

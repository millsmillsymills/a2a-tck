...
def test_certificate_chain_trust_validation(certificate_validation_info):
    ...
    is_self_signed = subject_cn == issuer_cn and all(subject.get(k) == issuer.get(k) for k in subject.keys())
    ...
    assert certificate_validation_info["validation_successful"], "Certificate chain validation failed during TLS handshake"

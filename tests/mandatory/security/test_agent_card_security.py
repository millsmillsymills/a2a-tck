            ...
            for sensitive_field in agent_card_security_info["sensitive_fields"]:
                if sensitive_field in public_card:
                    ...
                    if isinstance(field_value, str) and any(
                        ...
                        for indicator in ["secret", "private", "key", "token", "password"]
                    ):
                        pytest.fail(
                            f"Public Agent Card exposes potentially sensitive field '{sensitive_field}': {field_value}"
                        )
            ...
            try:
                ...
                if response.status_code not in (401, 403):
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Unexpected status for invalid {scheme_type} credentials ({scenario_name}): {response.status_code}")
                else:
                    logger.debug(f"✅ Invalid {scheme_type} credentials properly rejected ({scenario_name}): HTTP {response.status_code}")

...
def test_tls_protocol_version_security(tls_connection_info):
    ...
    version_info = tls_security_levels.get(tls_version, {
        "level": "unknown", 
        "recommendation": "Unknown TLS version - verify security"
    })
...
def test_certificate_chain_validation(tls_connection_info):
    ...
    hostname_match = (
        ...
        any(san.startswith('*.') and hostname.endswith(san[2:]) for san in san_list)
    )

    ...
    def test_security_scheme_structure_compliance(self, security_schemes):
        ...
        for i, scheme in enumerate(security_schemes):
            ...
            assert scheme_type in valid_scheme_types, \
                f"Security scheme {i} has invalid type: {scheme_type}"
            ...
            if scheme_type == "apiKey":
                ...
                assert scheme["in"] in valid_api_key_locations, \
                    f"API Key scheme {i} has invalid location: {scheme['in']}"
            
            elif scheme_type == "http":
                assert "scheme" in scheme, f"HTTP auth scheme {i} missing 'scheme' field"
                http_scheme = scheme["scheme"].lower()
                assert http_scheme in ["basic", "bearer", "digest"], \
                    f"HTTP auth scheme {i} has unsupported scheme: {http_scheme}"
                ...
                assert any(flow in flows for flow in valid_flows), \
                    f"OAuth2 scheme {i} has no valid flows"
            
            elif scheme_type == "openIdConnect":
                assert "openIdConnectUrl" in scheme, \
                    f"OpenID Connect scheme {i} missing 'openIdConnectUrl' field"
        ...
        for scheme in oauth2_schemes:
            if "oauth2MetadataUrl" in scheme:
                ...
                assert metadata_url.startswith("https://"), \
                    "OAuth2 metadata URL must use HTTPS"
    ...
    def test_authentication_transport_independence(self, sut_client, security_schemes):
        ...
        try:
            ...
            if "error" in response:
                ...
                if error.get("code") in [401, 403] or \
                   any(term in str(error.get("message", "")).lower() 
                       for term in ["auth", "unauthorized", "forbidden"]):
...
class TestEnhancedSecurityValidation:
    ...
    def test_transport_security_requirements(self, sut_client):
        ...
        assert sut_url.startswith("https://"), \
            "A2A v0.3.0 requires HTTPS for production deployments"

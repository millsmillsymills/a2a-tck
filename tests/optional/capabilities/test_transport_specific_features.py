...
class TestJSONRPCSpecificFeatures:
    ...
    def test_json_rpc_batch_requests(self, sut_client: BaseTransportClient):
        ...
        for i in range(3):
            ...
            req = message_utils.make_json_rpc_request(
                "message/send",
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(batch_response, list):
                    assert len(batch_response) == len(batch_requests), \
                        "Batch response length should match request length"
                    
                    # Each response should be valid JSON-RPC
                    for i, resp in enumerate(batch_response):
                        expected_id = f"batch-req-{i}"
                        assert ("result" in resp or "error" in resp), \
                            f"Batch response {i} should have result or error"
                        assert resp.get("id") == expected_id, \
                            f"Batch response {i} ID mismatch"
    ...
    def test_json_rpc_error_extensions(self, sut_client: BaseTransportClient):
        ...
        invalid_request = message_utils.make_json_rpc_request(
            "invalid/method",
        ...
        for content_type in content_types:
            ...
            try:
                ...
                if content_type == "application/json":
                    assert "application/json" in returned_content_type.lower(), \
                        "Should return JSON for JSON accept header"
...
class TestStreamingSpecificFeatures:
    ...
    async def test_sse_connection_management(self):
        ...
        request = message_utils.make_json_rpc_request(
            "message/stream",
            params={"message": message}
        )
...
class TestMultiModalFeatures:
    ...
    def test_file_reference_handling(self, sut_client: BaseTransportClient):
        ...
        if "result" in response:
            ...
            assert error.get("code") != -32601, \
                "File references should be supported in A2A v0.3.0"
    ...
    def test_binary_data_handling(self, sut_client: BaseTransportClient):
        ...
        if "result" in response:
            ...
            # Should not be method not found
            assert error.get("code") != -32601, \
                "Binary data should be supported in A2A v0.3.0"

...
def test_boundary_values(sut_client):
    ...
    large_history_req = message_utils.make_json_rpc_request("tasks/get", params={"id": task_id, "historyLength": 2147483647})
    ...
    zero_history_req = message_utils.make_json_rpc_request("tasks/get", params={"id": task_id, "historyLength": 0})
    zero_history_resp = transport_helpers.transport_get_task(sut_client, task_id, history_length=0)

    # Negative historyLength (invalid)
    neg_history_req = message_utils.make_json_rpc_request("tasks/get", params={"id": task_id, "historyLength": -1})
    ...
    assert transport_helpers.is_json_rpc_success_response(large_history_resp), "Large historyLength should be accepted"
    assert transport_helpers.is_json_rpc_success_response(zero_history_resp), "Zero historyLength should be accepted"

    # Negative should be rejected
    assert transport_helpers.is_json_rpc_error_response(neg_history_resp), "Negative historyLength should be rejected"

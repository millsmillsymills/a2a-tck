...
class JSONRPCError(TransportError):
    ...
    def __init__(self, message: str, original_error: Optional[Exception] = None,
                 json_rpc_error: Optional[Dict[str, Any]] = None):
...
class JSONRPCClient(BaseTransportClient):
    ...
    def _make_jsonrpc_request(self, method: str, params: Optional[Dict[str, Any]] = None,
    ...
    def raw_send(self, raw_data: str) -> Tuple[int, str]:
        ...
        try:
            response = self.session.post(
                self.base_url, data=raw_data, headers=headers, timeout=self.timeout
            )
            self._logger.info(f"SUT responded with {response.status_code}: {response.text}")
    ...
    def send_raw_json_rpc(self, json_request: dict) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                self.base_url, json=json_request, headers=headers, timeout=self.timeout
            )
            self._logger.info(f"SUT responded with {response.status_code}: {response.text}")
            response.raise_for_status()

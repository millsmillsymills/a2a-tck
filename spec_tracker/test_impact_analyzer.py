...
class TestImpactAnalyzer:
    ...
    def analyze_impact(self, spec_changes: Dict) -> Dict[str, List[str]]:
        ...
        for req_change in md_changes.get("requirements", {}).get("added", []):
            impact["new_coverage_needed"].extend(self._analyze_requirement_change(req_change, "added"))

        for req_change in md_changes.get("requirements", {}).get("removed", []):
            impact["obsolete_tests"].extend(self._analyze_requirement_change(req_change, "removed"))

        for req_change in md_changes.get("requirements", {}).get("modified", []):
            impact["directly_affected"].extend(self._analyze_requirement_change(req_change, "modified"))

        # Find tests affected by section changes
        for section_change in md_changes.get("sections", {}).get("added", []):
            impact["new_coverage_needed"].extend(self._analyze_section_change(section_change, "added"))

        for section_change in md_changes.get("sections", {}).get("modified", []):
            impact["directly_affected"].extend(self._analyze_section_change(section_change, "modified"))
        ...
        for method_change in json_changes.get("methods", {}).get("added", []):
            impact["new_coverage_needed"].extend(self._analyze_method_change(method_change, "added"))

        for method_change in json_changes.get("methods", {}).get("removed", []):
            impact["obsolete_tests"].extend(self._analyze_method_change(method_change, "removed"))

        # Find tests affected by definition changes
        for def_change in json_changes.get("definitions", {}).get("added", []):
            impact["new_coverage_needed"].extend(self._analyze_definition_change(def_change, "added"))

        # Find tests affected by error code changes
        for error_change in json_changes.get("error_codes", {}).get("modified", []):
            impact["directly_affected"].extend(self._analyze_error_change(error_change, "modified"))
    ...
    def _analyze_method_change(self, method_change: Dict, change_type: str) -> List[str]:
        ...
        for test_key, test_info in self.test_registry.items():
            ...
            try:
                with open(test_info["file"], encoding="utf-8") as f:
                    file_content = f.read().lower()
            except Exception:
                pass

            # Check if test uses this method
            if method_name.lower() in docstring or method_name.lower() in file_content:
                affected_tests.append(test_key)
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                        ...
                        test_info = {
                            ...
                            or any(
                                isinstance(d, ast.Name) and d.id == "pytest_asyncio"
    ...
    def _extract_spec_refs(self, docstring: str) -> List[str]:
        ...
        refs.extend([match.upper() if isinstance(match, str) else " ".join(match).upper() for match in requirement_matches])
        ...
        for pattern in protocol_patterns:
            ...
            if matches:
                refs.extend(
                    [f"Protocol: {match}" if isinstance(match, str) else f"Protocol: {' '.join(match)}" for match in matches]
                )
    ...
    def _extract_markers(self, node: ast.FunctionDef) -> List[str]:
        ...
        for decorator in node.decorator_list:
            if isinstance(decorator, ast.Call):
                ...
                if decorator.attr and isinstance(decorator.value, ast.Attribute) and decorator.value.attr == "mark":
    ...
    def find_tests_by_category(self, category: str) -> List[Dict]:
        ...
        return [test_info for test_info in self.test_registry.values() if test_info["category"] == category]
        ...
        for test_info in self.test_registry.values():
            for ref in test_info["spec_refs"]:
                if re.search(spec_ref_pattern, ref, re.IGNORECASE):
                    matching_tests.append({"test": test_info, "matching_ref": ref})
                    break
    ...
    def find_tests_without_spec_refs(self) -> List[Dict]:
        ...
        return [test_info for test_info in self.test_registry.values() if not test_info["spec_refs"]]
    ...
    def _calculate_requirement_priority(self, requirement: Any) -> int:
        ...
        priority_map = {"MUST": 10, "SHALL": 10, "REQUIRED": 9, "SHOULD": 7, "RECOMMENDED": 6, "MAY": 3, "OPTIONAL": 2}

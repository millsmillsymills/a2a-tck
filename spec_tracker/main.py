...
Examples:
  ...
  %(prog)s --summary-only                     # Generate summary report only
    ...
    parser.add_argument("--json-export", help="Export analysis results as JSON to specified file")
    ...
    try:
        ...
        try:
            ...
            with open(args.current_md, encoding="utf-8") as f:
                current_md = f.read()
        ...
        try:
            ...
            if total_changes == 0:
                ...
                if req_changes.get("added", 0) > 0 or req_changes.get("removed", 0) > 0:
                    logger.info(
                        f"  Requirements: +{req_changes.get('added', 0)}, -{req_changes.get('removed', 0)}, ~{req_changes.get('modified', 0)}"
                    )
        ...
        try:
            if args.summary_only:
                report = generator.generate_summary_report(spec_changes, test_impacts, coverage_analysis)
                report_type = "summary"
            else:
                report = generator.generate_report(
                    spec_changes, test_impacts, coverage_analysis, "Current A2A Specification", "Latest A2A Specification"
                )

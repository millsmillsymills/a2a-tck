...
def test_complete_pipeline_no_changes():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        report = generator.generate_report(
            comparison, 
            impact, 
            {"total_tests": 68, "tests_with_refs": 66, "documentation_coverage": 97.1},
            "Current", 
            "Latest"
        )
...
def test_complete_pipeline_with_changes():
    ...
    old_spec = {
        'markdown': parser.parse_markdown(old_md),
        'json': parser.parse_json_schema(old_json)
    }
...
def test_main_script_help():
    ...
    result = subprocess.run([
        sys.executable, 'spec_tracker/main.py', '--help'
...
def test_main_script_dry_run():
    ...
    result = subprocess.run([
        sys.executable, 'spec_tracker/main.py', '--dry-run'
    ], capture_output=True, text=True)
...
def test_error_handling():
    ...
    comparator = SpecComparator()
    empty_spec = {
        'markdown': empty_result,
        'json': empty_json_result
    }

...
class ReportGenerator:
    ...
    def generate_report(
        self, spec_changes: Dict, test_impacts: Dict, coverage_analysis: Dict, old_version: str, new_version: str
    ) -> str:
        ...
        # Header
        report.append("# A2A Specification Change Analysis Report")
    ...
    def _generate_summary(self, spec_changes: Dict, test_impacts: Dict, coverage_analysis: Dict) -> List[str]:
        ...
        if def_added + def_removed + def_modified > 0:
            summary.append(
                f"- **JSON Schema Changes**: {def_added} definitions added, {def_removed} removed, {def_modified} modified"
            )
    ...
    def _format_spec_changes(self, spec_changes: Dict) -> List[str]:
        ...
        if added_reqs:
            ...
            for req_change in added_reqs[:10]:  # Limit to first 10
                ...
                text = (
                    getattr(req, "text", str(req))[:100] + "..."
                    if len(getattr(req, "text", str(req))) > 100
                    else getattr(req, "text", str(req))
                )
        ...
        if added_defs:
            ...
            for def_change in added_defs[:10]:  # Limit to first 10
                ...
                desc = (
                    definition.get("description", "No description")[:80] + "..."
                    if len(definition.get("description", "")) > 80
                    else definition.get("description", "No description")
                )
                changes.append(f"\n- **{name}**: {desc}")
        ...
        if added_methods:
            ...
            for method_change in added_methods:
                ...
                desc = (
                    method_info.get("description", "No description")[:80] + "..."
                    if len(method_info.get("description", "")) > 80
                    else method_info.get("description", "No description")
                )
                changes.append(f"\n- **{name}**: {desc}")
        ...
        if uncovered_reqs:
            ...
            if high_priority:
                ...
                for req_info in high_priority[:5]:
                    ...
                    text = (
                        req_info.get("text", "")[:80] + "..." if len(req_info.get("text", "")) > 80 else req_info.get("text", "")
                    )
    ...
    def _generate_recommendations(self, spec_changes: Dict, test_impacts: Dict, coverage_analysis: Dict) -> List[str]:
        ...
        if breaking_changes:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Review all breaking changes before deploying")
            recommendations.append("2. Update client code that depends on removed/changed APIs")
            recommendations.append("3. Run full test suite to identify failures")
            recommendations.append("4. Update documentation to reflect changes")
        ...
        if obsolete_tests:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Review each obsolete test to confirm it's no longer needed")
            recommendations.append("2. Remove or update tests that test removed functionality")
            recommendations.append("3. Archive removed tests with documentation explaining why")
        ...
        if new_coverage_needed:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Create tests for new MUST and SHALL requirements (highest priority)")
            recommendations.append("2. Add tests for new SHOULD requirements (medium priority)")
            recommendations.append("3. Consider edge cases and error conditions for new features")
            recommendations.append("4. Update test documentation with new specification references")
        ...
        if directly_affected:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Review each affected test for accuracy")
            recommendations.append("2. Update test expectations if behavior has changed")
            recommendations.append("3. Update test documentation to reflect spec changes")
            recommendations.append("4. Run affected tests to ensure they still pass")
        ...
        if high_priority_uncovered:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Create tests for uncovered MUST/SHALL requirements")
            recommendations.append("2. Focus on core protocol functionality and error handling")
            recommendations.append("3. Add negative test cases for requirement violations")
        ...
        if tests_without_refs:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Add docstrings with specification references to undocumented tests")
            recommendations.append("2. Use format: 'Tests A2A Specification §X.Y requirement that...'")
            recommendations.append("3. Link tests to specific MUST/SHOULD/MAY requirements")
        ...
        if test_doc_percentage < 90:
            ...
            recommendations.append("\n**Action Items:**")
            recommendations.append("1. Add specification references to all test docstrings")
            recommendations.append("2. Document the specific requirement or behavior being tested")
            recommendations.append("3. Include section references (A2A §X.Y) in test descriptions")
        ...
        if total_changes == 0:
            ...
            recommendations.append("1. Review test coverage for completeness")
            recommendations.append("2. Update test documentation where missing")
            recommendations.append("3. Consider adding tests for edge cases")
            recommendations.append("4. Run periodic compliance checks")
        else:
            recommendations.append("\n**Active specification evolution detected** - implement change management:")
            recommendations.append("1. Set up automated spec change detection")
            recommendations.append("2. Create a review process for specification updates")
            recommendations.append("3. Maintain a change log linking spec changes to test updates")
            recommendations.append("4. Consider backward compatibility implications")

        # Timeline recommendations
        recommendations.append("\n### ⏰ Recommended Timeline")

        priority_score = (
            len(breaking_changes) * 10 + len(obsolete_tests) * 5 + len(directly_affected) * 2 + len(new_coverage_needed)
        )

        if priority_score >= 50:
            recommendations.append("\n**High Priority (Complete within 1-2 weeks):**")
            recommendations.append("- Address all breaking changes immediately")
            recommendations.append("- Review and update obsolete tests")
            recommendations.append("- Run full test suite to identify issues")
        elif priority_score >= 20:
            recommendations.append("\n**Medium Priority (Complete within 1 month):**")
            recommendations.append("- Review affected tests and update as needed")
            recommendations.append("- Add tests for new requirements")
            recommendations.append("- Update test documentation")
        else:
            recommendations.append("\n**Low Priority (Complete within 2-3 months):**")
            recommendations.append("- Improve test coverage gradually")
            recommendations.append("- Enhance test documentation")
            recommendations.append("- Consider adding edge case tests")

        # Quality metrics to track
        recommendations.append("\n### 📊 Quality Metrics to Track")
        recommendations.append("\n**Monitor these metrics over time:**")
        recommendations.append("- Requirement coverage percentage (target: 95%+)")
        recommendations.append("- Test documentation percentage (target: 95%+)")
        recommendations.append("- Breaking change impact (minimize affected tests)")
        recommendations.append("- Time to update tests after spec changes (target: <1 week)")
    ...
    def generate_summary_report(self, spec_changes: Dict, test_impacts: Dict, coverage_analysis: Dict) -> str:
        ...
        summary.append("# A2A Spec Change Summary")
